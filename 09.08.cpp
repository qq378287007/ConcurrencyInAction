...
public:
    thread_pool() : joiner(threads), done(false)
    {
        ...
        try
        {
            ...
            {
                queues.push_back(unique_ptr<work_stealing_queue>(
                    new work_stealing_queue));
                threads.push_back(
                    thread(&thread_pool::worker_thread, this, i));
            }
        }
        catch (...)
        {
    ...
    using task_handle = unique_future<ResultType>;

    template <typename FunctionType>
    task_handle<result_of<FunctionType()>::type> submit(
        FunctionType f)
    {
        typedef result_of<FunctionType()>::type result_type;
